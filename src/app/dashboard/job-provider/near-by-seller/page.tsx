...
import { useNearBySeekers } from "@/hooks/useNearBySeeker";
import NearBySeekerPagination from "@/components/near-by-seeker/NearBySeekerPagination";
...
export default function NearBySellerPage() {
    const router = useRouter();
    const { latitude, longitude } = useUserLocation();

    const [currentPage, setCurrentPage] = useState<number>(1);

    const { users: jobSeekers, pagination, isLoading, isError, mutate } = useNearBySeekers(currentPage, latitude || undefined, longitude || undefined);
                    ...
                    <div className="lg:col-span-6 py-6">
                        ...
                        <div className="mb-6 flex items-center justify-between ">
                            ...
                            >
                                ...
                                <h1 className="text-xl font-bold text-gray-900">Near by Sellers</h1>
                            </button>
                            <div>
                            ...
                            {isLoading ? (
                                ...
                                <>
                                    {jobSeekers.length === 0 ? (
                                        ...
                                        <div className="space-y-4">
                                            {jobSeekers.map((seeker: JobSeekerI) => (
                                                <Link
                                                    key={seeker._id}
                                                    href={`/dashboard/job-provider/profile/user/${seeker._id}`}
                                                >
                                                    <PeopleCard
                                                        key={seeker._id}
                                                        data={seeker}
