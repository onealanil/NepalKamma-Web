...
export interface JobSeekerPaginationInfo {
    currentPage: number;
    totalPages: number;
...
export interface TopRatedJobSeekerPaginationProps {
    pagination: JobSeekerPaginationInfo;
    currentPage: number;
    onPageChange: (page: number) => void;
    isLoading?: boolean;
}

export default function TopRatedJobSeekerPagination({
    pagination,
    currentPage,
    onPageChange,
    isLoading = false
}: TopRatedJobSeekerPaginationProps) {
    const { totalPages, hasNextPage, hasPrevPage, totalJobSeekers } = pagination;
    ...
    return (
        <div className="flex flex-col gap-4 z-50 items-center justify-between mt-6 pt-4 border-t border-black">
            <div className="text-sm text-gray-600">
                Showing page {currentPage} of {totalPages} ({totalJobSeekers} total job seekers)
            </div>
