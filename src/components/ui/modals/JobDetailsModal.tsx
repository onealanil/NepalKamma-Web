...
export const JobDetailsModal = ({
  ...
  const [isJobUpdating, setIsJobUpdating] = useState<boolean>(false);
  ...
  const handleSave = async (status: string, users: User[]) => {
    ...
    if (response.success) {
      if (status === "Cancelled") {
        ...
        SuccessToast(response?.message || "Job updated successfully!");
      }
      mutate();
      setIsEditOpen(false);
      onClose();
...
v-3a1 1 0 00-1-1H9z" clipRule="evenodd" />
                    ...
                    <div className="ml-3">
                      <p className="text-sm text-blue-700">
                        Click on update status button to assign a freelancer to this job or change the job status.
                      </p>
                      <ol>
                        <li className="text-sm text-blue-700 mt-1">1. Pending - The job is awaiting assignment.</li>
                        <li className="text-sm text-blue-700 mt-1">2. If you want to assign a freelancer, select &quot;In Progress&quot; status and search the user with their username.</li>
                        <li className="text-sm text-blue-700 mt-1">3. If the job is done, select &quot;Completed&quot; status to mark it as finished.</li>
                      </ol>
                    </div>
                  </div>
                </div>
